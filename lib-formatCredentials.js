...
function splitDelimiters(line, expectedCount) {
  let t = clean(line);
  if (!t) return [];
  // Strip leading "1." / "2." numbering from multi-log display strings
  t = t.replace(/^\d+\.\s+/, '');
  ...
  // Single-space separated: user pass 2fa (Sujan / many suppliers)
  const words = t.split(/\s+/).map(stripQuotes).filter(Boolean);
  if (words.length >= 2) {
    if (expectedCount && expectedCount >= 2 && words.length >= expectedCount) {
      // Prefer exact field count from format hint (Username|password|2fa → 3)
      if (words.length === expectedCount) return words;
      // More tokens than fields: first N-1 tokens, rest joined as last field (password with spaces rare)
      return words.slice(0, expectedCount - 1).concat(words.slice(expectedCount - 1).join(' '));
    }
    if (words.length >= 2 && words.length <= 8) return words;
  }
  return [t];
}
...
export function formatOneAccount(raw, formatHint) {
  ...
  if (parts && parts.length >= 2) {
    ...
    if (hinted && (hinted.username || hinted.password || hinted.email || hinted.token)) {
