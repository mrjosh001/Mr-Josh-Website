...
/** 3% lifetime referral on deposits — shared logic (kept here so no extra serverless file) */
...
async function userUpdate(body) {
  ...
  if (amountAdded > 0) {
    ...
    // Lifetime 3% to referrer when admin manually funds a referred user
    try {
    ...
    // Admin balance corrections (deductions) update the wallet only —
    // do NOT write a "Balance correction / Manual Adjustment" row into
...
async function getUserJoinDates() {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    for (const u of users) {
      ...
      if (u.id) {
        // Supabase Auth: confirmed when email_confirmed_at is set
        emailVerified[u.id] = !!(u.email_confirmed_at || u.confirmed_at);
      }
    }
