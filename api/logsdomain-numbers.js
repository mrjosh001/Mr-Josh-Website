...
async function handleSync(req, res) {
  ...
  while (cursor < countryList.length && (Date.now() - startedAt) < SYNC_TIME_BUDGET_MS) {
    ...
    try {
      ...
      // Deduplicate by service_id before inserting. LogsDomain's services
      // list sometimes contains two entries sharing the same service_id
      // within a single country — a plain multi-row INSERT can't have two
      // rows targeting the same unique key, so that one duplicate was
      // silently failing the ENTIRE 200-row batch it landed in, dropping
      // up to 199 unrelated legitimate services along with it (some
      // countries ended up with 1 row instead of hundreds because of this).
      const dedupedInsert = Array.from(
        new Map(toInsert.map((row) => [row.service_id, row])).values()
      );

      for (let i = 0; i < dedupedInsert.length; i += 200) {
        const batch = dedupedInsert.slice(i, i + 200);
        // upsert (not insert) as a second layer of defense: if a resumed
        // run or an overlapping cron tick already inserted one of these
        // rows between our existence check above and this write, upsert
        // resolves it instead of failing the whole batch again.
        const { error } = await supabase
          .from('number_services')
          .upsert(batch, { onConflict: 'source,country_id,service_id' });
