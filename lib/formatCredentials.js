/**
 * Normalize supplier log lines for customer view.
 * URLs are never split on ":". Fields are classified by content, not only order.
 */

function clean(s) {
  return String(s || '').replace(/\r/g, '').replace(/\u0000/g, '').trim();
}
function stripQuotes(s) {
  return clean(s).replace(/^["']|["']$/g, '');
}

function isEmail(s) {
  return /^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$/.test(String(s || '').trim());
}
function isUrl(s) {
  return /^https?:\/\//i.test(String(s || '').trim());
}
function isPhone(s) {
  const raw = String(s || '').trim();
  // Bare 10–15 digit IDs are Facebook/UIDs, not phone numbers
  if (/^\d{8,}$/.test(raw)) return false;
  // +E.164 or US-style (908) 280-4647 / 908-280-4647
  if (/^\+?\d[\d\s().-]{8,18}\d$/.test(raw) && raw.replace(/\D/g, '').length >= 10 && raw.replace(/\D/g, '').length <= 15) {
    // Reject pure UIDs that only happen to be digits with no separators and no +
    if (/^\d{8,}$/.test(raw) && !/^\+/.test(raw)) return false;
    if (/[()\-]/.test(raw) || /^\+/.test(raw) || /\s/.test(raw)) return true;
  }
  const t = raw.replace(/[\s()-]/g, '');
  return /^\+\d{10,15}$/.test(t);
}

function protectUrls(text) {
  const urls = [];
  let s = String(text || '').replace(/:(https?:\/\/)/gi, '|$1');
  s = s.replace(/https?:\/\/[^\s|;,]+/gi, (m) => {
    const u = m.replace(/[)\].,;]+$/g, '');
    const i = urls.length;
    urls.push(u);
    return `__URL${i}__`;
  });
  return { text: s, urls };
}
function restoreUrls(text, urls) {
  let t = String(text || '');
  for (let i = 0; i < urls.length; i++) t = t.split(`__URL${i}__`).join(urls[i]);
  return t;
}

function isLikely2FA(s) {
  const t = String(s || '').trim();
  if (!t || t.length < 10) return false;
  if (/@/.test(t) || isUrl(t)) return false;
  // Passwords with % ! @ # $ or similar specials are not TOTP secrets
  if (/[%!#$^*()+=\[\]{}]/.test(t) && t.length < 24) return false;
  if (isCookieBlob(t)) return false;
  // Spaced groups: "5vjq 3rlu ilmz jpyk 7flc i6xq hp7c qren"
  if (/^(?:[A-Za-z0-9]{3,5}\s+){3,}[A-Za-z0-9]{2,8}$/.test(t)) {
    const spaced = t.replace(/\s+/g, '');
    if (spaced.length >= 12 && spaced.length <= 64) return true;
  }
  if (/^(?:[A-Za-z0-9]{4}\s+){5,}[A-Za-z0-9]{4}$/.test(t)) return true;
  const compact = t.replace(/[\s-]/g, '');
  if (compact.length < 12 || compact.length > 80) return false;
  const lowerN = (compact.match(/[a-z]/g) || []).length;
  const upperN = (compact.match(/[A-Z]/g) || []).length;
  // Usernames like rebetrigomar79943874 are mostly lowercase — not 2FA
  if (lowerN > upperN) return false;
  if (/^[A-Z2-7]{12,64}$/.test(compact)) return true;
  if (/^[A-Z0-9]{12,64}$/.test(compact) && upperN > 0 && compact === compact.toUpperCase()) return true;
  if (/^[A-Za-z0-9]{16,64}$/.test(compact) && upperN >= 8 && /[0-9]/.test(compact)) return true;
  if (/^(?:[A-Za-z2-7]{4}\s+){2,}[A-Za-z2-7]{2,8}$/.test(t) && compact.length >= 12) return true;
  const letters = compact.replace(/[^A-Za-z]/g, '');
  if (letters.length >= 12) {
    const up = (letters.match(/[A-Z]/g) || []).length;
    if (up / letters.length >= 0.7 && compact.length >= 14) return true;
  }
  return false;
}

function isCookieBlob(s) {
  const t = String(s || '').trim();
  if (t.length < 20) return false;
  if (/c_user=|xs=|datr=|sb=|fr=|sessionid=|auth_token=|li_at=/i.test(t)) return true;
  if (/^eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+/.test(t)) return true; // JWT
  if (t.split(';').length >= 3 && /=/.test(t) && t.length > 40) return true;
  return false;
}

function mapToken(p) {
  const u = String(p || '').toUpperCase().replace(/\s+/g, ' ').trim();
  if (/^(USER NAME|USERNAME|USER|LOGIN|ID|UID|ACCOUNT)$/.test(u)) return 'username';
  if (/^(PASS WORD|PASSWORD|PASS|PWD)$/.test(u)) return 'password';
  if (/^(EMAIL PASS(WORD)?|MAIL PASS(WORD)?)$/.test(u)) return 'email_password';
  if (/^(E-?MAIL|MAIL|EMAIL)$/.test(u)) return 'email';
  if (/^(2 FA|2FA|2FA CODE|TWO FA|TOTP|OTP SECRET|SECRET)$/.test(u)) return 'token';
  if (/^(TOKEN|COOKIE|COOKIES|AUTH TOKEN)$/.test(u)) return 'cookie';
  if (/^(PHONE|MOBILE|NUMBER)$/.test(u)) return 'phone';
  if (/^(RECOVERY|RECOVERY MAIL|RECOVERY EMAIL|BACKUP MAIL)$/.test(u)) return 'extra';
  return null;
}

export function parseFormatHint(description) {
  if (!description) return null;
  const text = clean(description);
  let focus = text;
  const fmtBlock = text.match(/(?:delivery\s*format|format|credentials?\s*format)\s*[:\-–]?\s*([^\n]+)/i);
  if (fmtBlock) {
    focus = fmtBlock[1];
    const pairOnly = focus.match(
      /^\s*((?:Number|Phone|Mobile|Username|User|Email\s*Password|Password|Pass|Email|Uid|Login)(?:\s*[\/|]\s*(?:Number|Phone|Mobile|Username|User|Email\s*Password|Password|Pass|Email|Uid|Login|2FA|Token))+)/i
    );
    if (pairOnly) focus = pairOnly[1];
  }
  // Only use compact "UID|PASS|2FA" slash lists when that is the whole format.
  // Do NOT shrink "Facebook UID | Facebook Password | 2FA CODE | ..." down to "Password | 2FA".
  const slash = text.match(
    /\b((?:USER(?:NAME)?|PASS(?:WORD)?|EMAIL(?:\s*PASS(?:WORD)?)?|2FA|TOKEN|COOKIE|LOGIN|UID|NUMBER|PHONE|MOBILE)(?:\s*[\/|]\s*(?:USER(?:NAME)?|PASS(?:WORD)?|EMAIL(?:\s*PASS(?:WORD)?)?|2FA|TOKEN|COOKIE|LOGIN|UID|NUMBER|PHONE|MOBILE))+)\b/i
  );
  if (slash && /[\/|]/.test(slash[1])) {
    const slashTokens = slash[1].split(/[\/|]/).map((x) => x.trim()).filter(Boolean);
    const focusTokens = (focus.match(/UID|USER(?:NAME)?|PASS(?:WORD)?|2\s*FA|2FA|EMAIL|COOKIE|TOKEN|LOGIN|NUMBER|PHONE|MOBILE/gi) || []).length;
    if (!fmtBlock || slashTokens.length >= Math.max(3, focusTokens)) focus = slash[1];
  }
  // Prefer separate Email + Password when both words appear (not "Email Password" as one field)
  // so "Email Password 2FA" → email, password, token — not email_password, token
  const separateEmailPass = /\bemail\b[\s\/|,]+(?:pass(?:word)?)\b/i.test(focus)
    && !/\bemail\s*pass(?:word)?\b/i.test(focus.replace(/\bemail\b[\s\/|,]+pass(?:word)?\b/gi, ''));
  // "Email Password 2FA" style with spaces between three labels
  const threeLabel = focus.match(/\bemail\b\s+\bpass(?:word)?\b\s+(?:2\s*fa|2fa|token)\b/i);
  const tokenRe = threeLabel || (/\bemail\b\s+\bpass(?:word)?\b/i.test(focus) && /\b(2\s*fa|2fa|token)\b/i.test(focus))
    ? /User\s*name|Username|Password|Login|Email|User|Pass|2\s*FA\s*CODE|2\s*FA|2FA|Token|Cookies?|Phone|Mobile|Number|Uid|ID/gi
    : /Email\s*Password|Recovery\s*mail|User\s*name|Username|Password|Login|Email|User|Pass|2\s*FA\s*CODE|2\s*FA|2FA|Token|Cookies?|Phone|Mobile|Number|Uid|ID/gi;
  const found = [];
  let m;
  const re = new RegExp(tokenRe.source, 'gi');
  while ((m = re.exec(focus)) !== null) {
    const key = mapToken(m[0]);
    if (key && !found.includes(key)) found.push(key);
  }
  return found.length >= 2 ? found : null;
}

function classifyPart(p) {
  const s = stripQuotes(p);
  if (!s) return { kind: 'empty', value: s };
  if (isUrl(s)) return { kind: 'url', value: s };
  if (isEmail(s)) return { kind: 'email', value: s };
  if (isCookieBlob(s)) return { kind: 'cookie', value: s };
  if (isLikely2FA(s)) return { kind: 'token', value: s.replace(/\s+/g, '') };
  if (isPhone(s)) return { kind: 'phone', value: s };
  if (/^\d{8,}$/.test(s)) return { kind: 'uid', value: s };
  if (s.length >= 2 && s.length <= 64 && !/\s/.test(s)) return { kind: 'secret', value: s };
  return { kind: 'text', value: s };
}

function fieldsFromClassified(parts) {
  const f = {};
  const extras = [];
  let passwordTaken = false;
  for (const raw of parts) {
    const { kind, value } = classifyPart(raw);
    if (kind === 'empty') continue;
    // Cookie/session blobs are never a password
    if (kind === 'cookie') {
      f.token = f.token ? f.token + ' | ' + value : value;
      continue;
    }
    if (kind === 'email' && !f.email) f.email = value;
    else if (kind === 'uid' && !f.username) f.username = value;
    else if (kind === 'phone' && !f.phone) f.phone = value;
    else if (kind === 'token' && !f.token) f.token = value;
    else if (kind === 'url') extras.push(value);
    else if (kind === 'secret') {
      // Long cookie-like secrets → token
      if (isCookieBlob(value)) {
        f.token = f.token ? f.token + ' | ' + value : value;
      } else if (!f.username && !f.email) f.username = value;
      else if (!passwordTaken) {
        f.password = value;
        passwordTaken = true;
      } else extras.push(value);
    } else if (kind === 'text') {
      if (isCookieBlob(value)) {
        f.token = f.token ? f.token + ' | ' + value : value;
      } else if (!f.username && !isUrl(value) && value.length <= 80) f.username = value;
      else if (!passwordTaken && value.length >= 2 && value.length <= 64 && !/\s/.test(value)) {
        f.password = value;
        passwordTaken = true;
      } else extras.push(value);
    } else extras.push(value);
  }
  if (extras.length) f.extra = extras.join(' | ');
  return f;
}

function splitDelimiters(line, expectedCount) {
  let t = clean(line);
  if (!t) return [];
  t = t.replace(/^\d+\.\s+/, '');
  const { text: protectedText, urls } = protectUrls(t);
  t = protectedText;
  const restoreParts = (parts) =>
    parts.map((p) => restoreUrls(stripQuotes(p), urls)).filter((p) => p.length > 0);

  if (t.includes('|')) {
    let segs = t.split('|').map(stripQuotes).filter(Boolean);
    if (segs.length >= 1 && segs[0].includes(':') && !/^__URL\d+__$/.test(segs[0])) {
      const head = segs[0];
      const em = head.match(/^([^\s:<>]+@[^\s:<>]+)\s*:\s*(.+)$/);
      if (em) segs = [em[1], em[2], ...segs.slice(1)];
      else {
        const colonParts = head.split(':').map(stripQuotes).filter(Boolean);
        if (colonParts.length >= 2) segs = [...colonParts, ...segs.slice(1)];
      }
    }
    return restoreParts(segs);
  }
  if (t.includes(';') && t.split(';').filter(Boolean).length >= 2 && !isCookieBlob(restoreUrls(t, urls))) {
    return restoreParts(t.split(';'));
  }

  const emailHead = t.match(/^([^\s:<>]+@[^\s:<>]+)\s*:\s*(.+)$/);
  if (emailHead) {
    const email = emailHead[1];
    const rest = emailHead[2];
    const segs = rest.split(':').map(stripQuotes).filter(Boolean);
    return restoreParts([email, ...segs]);
  }

  if (t.includes(':')) {
    const parts = t.split(':').map(stripQuotes).filter((p) => p.length > 0);
    if (parts.length >= 2) {
      if (expectedCount && expectedCount >= 2 && parts.length > expectedCount) {
        const head = parts.slice(0, expectedCount - 1);
        const tailParts = parts.slice(expectedCount - 1);
        const tailHasUrl = tailParts.some((x) => /__URL\d+__/.test(x) || /^https?:/i.test(x));
        if (tailHasUrl) return restoreParts([...head, ...tailParts]);
        return restoreParts([...head, tailParts.join(':')]);
      }
      return restoreParts(parts);
    }
  }

  // Number / Password  e.g. +15743752203/Importance1.
  {
    const phoneSlash = restoreUrls(t, urls).match(
      /^\s*(\+?\d[\d\s().-]{8,16}\d)\s*[\/|:]\s*([^\s\/][^\n]{0,80}?)\s*$/
    );
    if (phoneSlash && isPhone(phoneSlash[1].trim()) && !isUrl(phoneSlash[2].trim())) {
      return [phoneSlash[1].trim(), phoneSlash[2].trim()];
    }
  }

  // Space-separated unlabeled tokens first (uid pass 2FA url)
  const spaced = splitSpacedTokens(restoreUrls(t, urls));
  if (spaced && spaced.length >= 2) return spaced;

  const prose = parseProseCredential(restoreUrls(t, urls));
  if (prose && prose.length >= 2) return prose;

  return restoreParts([t]);
}

function stitchUsPhone(text) {
  let s = String(text || '');
  s = s.replace(
    /\+?1\s*[\/|]?\s*\(\s*(\d{3})\s*\)\s*[\/|]?\s*(\d{3})\s*[-.\s\/]?\s*(\d{4})\b/g,
    '+1 ($1) $2-$3'
  );
  s = s.replace(
    /\+?1\s*[\/|]\s*(\d{3})\s*[\/|]\s*(\d{3})\s*[-.\s]?\s*(\d{4})\b/g,
    '+1 ($1) $2-$3'
  );
  return s;
}

function repairSplitUsLabeled(text) {
  const s = String(text || '');
  const area = s.match(/Password:\s*\(?\s*(\d{3})\s*\)?\s*$/im);
  const rest = s.match(/Extra:\s*(\d{3})\s*[-.\s]\s*(\d{4})\s*[\/|:]\s*(.+)$/im);
  const num = s.match(/Number:\s*\+?1\s*$/im);
  if (!num || !area || !rest) return null;
  return fieldsToText({
    phone: '+1 (' + area[1] + ') ' + rest[1] + '-' + rest[2],
    password: String(rest[3] || '').trim() || undefined
  });
}

function splitSpacedTokens(text) {
  const raw = clean(text);
  if (!raw) return null;
  const { text: prot, urls } = protectUrls(raw);
  const LABEL = /^(uid|user(?:name)?|login|id|pass(?:word)?|pwd|email|mail|token|2fa|link|url|number)$/i;
  let tokens = prot
    .split(/\s+/)
    .map((x) => restoreUrls(stripQuotes(x), urls))
    .filter((x) => x && !LABEL.test(x));
  tokens = tokens.reduce((acc, tok) => {
    const prev = acc[acc.length - 1] || '';
    const prev2 = acc[acc.length - 2] || '';
    if (/^\(\d{3}\)$/.test(tok) && /^\+?1$/.test(prev)) {
      acc[acc.length - 1] = prev + ' ' + tok;
      return acc;
    }
    if (/^\d{3}[-.\s]\d{4}/.test(tok) && /\(\d{3}\)$/.test(prev) && /^\+?1/.test(prev2 + prev)) {
      acc[acc.length - 1] = prev + ' ' + tok;
      return acc;
    }
    acc.push(tok);
    return acc;
  }, []);
  if (tokens.length < 2) return null;
  // Need at least two classifiable pieces (email/uid/secret/token/url)
  let useful = 0;
  for (const tok of tokens) {
    const k = classifyPart(tok).kind;
    if (k !== 'empty' && k !== 'text') useful += 1;
    else if (k === 'text' && tok.length >= 2 && tok.length <= 64) useful += 1;
  }
  if (useful < 2) return null;
  return tokens;
}

function parseProseCredential(text) {
  const t = clean(text);
  if (!t || t.length < 6) return null;
  const passM = t.match(/\b(?:password|pass|pwd)\s*[:=\-]?\s*(\S+)/i);
  const uidM =
    t.match(/\b(?:uid|user(?:name)?|login|id)\s*(?:number)?\s*[:=\-]?\s*(\d{6,})\b/i) ||
    t.match(/\b(\d{10,})\b/);
  const emailM = t.match(/\b([^\s@<>]+@[^\s@<>]+\.[^\s@<>]+)\b/);
  const { urls } = protectUrls(t);
  const flat = t.replace(/[\n\r]+/g, ' ');
  const spaced2fa = flat.match(/\b(?:[A-Z2-7]{2,8}(?:\s+[A-Z0-9]{2,8}){2,})\b/);
  const compact2fa = t.match(/\b([A-Z2-7]{12,64})\b/);
  const links = urls.length ? urls : t.match(/https?:\/\/[^\s]+/gi) || [];
  if (!passM && !uidM && !emailM && links.length < 1 && !spaced2fa) return null;
  const parts = [];
  if (emailM) parts.push(emailM[1]);
  if (uidM) parts.push(uidM[1]);
  if (passM && !/^(https?)$/i.test(passM[1])) parts.push(passM[1]);
  if (spaced2fa) parts.push(spaced2fa[0].replace(/\s+/g, ''));
  else if (compact2fa && isLikely2FA(compact2fa[1])) parts.push(compact2fa[1]);
  for (const u of links) if (!parts.includes(u)) parts.push(u);
  return parts.length >= 2 ? parts : null;
}


function pretty2fa(s) {
  const compact = String(s || '').replace(/\s+/g, '').toUpperCase();
  if (!compact) return s;
  // License / activation codes with dashes — keep original form
  if (/-/.test(String(s || '')) && /^[A-Z0-9-]{8,40}$/i.test(String(s || '').replace(/\s+/g, ''))) {
    return String(s).replace(/\s+/g, '').toUpperCase();
  }
  return compact.match(/.{1,4}/g).join(' ');
}
function tokenFrom2faUrl(s) {
  const m = String(s || '').match(/2fa\.live\/([A-Za-z0-9]{10,80})/i);
  return m ? m[1].replace(/\s+/g, '').toUpperCase() : '';
}
function polishLabeled(labeled, original) {
  const f = { ...labeled };
  const blob = [original, f.extra, f.token, f.password, f.username].filter(Boolean).join('\n');
  if (f.token && isUrl(String(f.token))) {
    const fromTok = tokenFrom2faUrl(f.token);
    if (fromTok) {
      f.extra = [f.token, f.extra].filter(Boolean).join(' | ');
      f.token = fromTok;
    }
  }
  if (!f.token) {
    const fromUrl = tokenFrom2faUrl(blob);
    if (fromUrl) f.token = fromUrl;
  }
  if (!/^(?:Email|Username|Password|2FA)\s*:/im.test(String(original || ''))) {
    const { text: prot, urls } = protectUrls(String(original || '').replace(/\n+/g, ' '));
    const bits = prot.split(/[:|]+/).map((x) => restoreUrls(stripQuotes(x), urls)).filter(Boolean);
    if (bits.length >= 2 && !isUrl(bits[0]) && !/^https?$/i.test(bits[0])) {
      const labelLike = /^(email|username|user|password|pass|2fa|token|extra)$/i;
      if (!f.username && !f.email && !isLikely2FA(bits[0]) && !labelLike.test(bits[0])) {
        if (isEmail(bits[0])) f.email = bits[0];
        else f.username = bits[0];
      }
      if (!f.password && bits[1] && !isUrl(bits[1]) && !isLikely2FA(bits[1]) && !labelLike.test(bits[1])) {
        f.password = bits[1];
      }
    }
  }
  if (f.token) {
    const rawTok = String(f.token);
    if (isCookieBlob(rawTok) || /c_user=|xs=|datr=|wL_cbv=/i.test(rawTok) || (rawTok.length > 80 && /;/.test(rawTok) && /=/.test(rawTok))) {
      // keep cookie intact — never pretty2fa
      f.token = rawTok.trim();
    } else {
      const compact = rawTok.replace(/\s+/g, '').toUpperCase();
      const extra = String(f.extra || '');
      if (/2fa\.live\//i.test(extra) && extra.toUpperCase().includes(compact)) {
        delete f.extra;
      }
      f.token = pretty2fa(compact);
    }
  }
  return f;
}
function extractLabeled(text) {
  const out = {};
  const { text: protectedText, urls } = protectUrls(clean(text));
  const patterns = [
    ['username', /(?:^|\n)\s*(?:user\s*name|username|user\s*id|\blogin\b|\buid\b|\bid\b)\s*[:=\-]\s*(.+)/i],
    ['password', /(?:^|\n)\s*(?:pass\s*word|password|\bpwd\b)\s*[:=\-]\s*(.+)/i],
    ['email_password', /(?:^|\n)\s*(?:email\s*pass(?:word)?|mail\s*pass(?:word)?)\s*[:=\-]\s*(.+)/i],
    ['recovery', /(?:^|\n)\s*(?:recovery\s*(?:e-?mail|mail)|backup\s*(?:e-?mail|mail)|recovery)\s*[:=\-]\s*(.+)/i],
    // Match "Email:" / "E-mail" / "E -mail" before bare "mail" so the label is not a value
    ['email', /(?:^|\n)\s*(?:e-?mail\s*address|e\s*[-–—]?\s*mail\b)\s*[:=\-]?\s*(.+)/i],
    ['token', /(?:^|\n)\s*(?:2\s*fa\s*\/\s*token|auth\s*token|\btoken\b|\bcookie\b|2\s*fa|\b2fa\b|otp\s*secret|\bcode\b)\s*[:=\-]\s*(.+)/i],
    ['phone', /(?:^|\n)\s*(?:\bphone\b|\bmobile\b|\bnumber\b)\s*[:=\-]\s*(.+)/i],
    ['year', /(?:^|\n|\s)\s*(?:\byear\b)\s*[:=\-]\s*(\d{4})\b/i],
    ['location', /(?:^|\n|\s)\s*(?:\blocation\b)\s*[:=\-]\s*(.+)/i],
    ['plan', /(?:^|\n|\s)\s*(?:\bplan\b)\s*[:=\-]\s*(.+)/i],
    ['autorenew', /(?:^|\n|\s)\s*(?:auto\s*renew)\s*[:=\-]\s*(.+)/i],
    ['expiredate', /(?:^|\n|\s)\s*(?:expire\s*date|expiry(?:\s*date)?|expires?)\s*[:=\-]\s*(.+)/i],
    ['daysleft', /(?:^|\n|\s)\s*(?:days?\s*left)\s*[:=\-]\s*(.+)/i]
  ];
  const lines = protectedText.split(/\n+/);
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    // Bare email alone on a line (Google Voice / mail logs)
    if (!out.email) {
      const bare = line.trim().match(/^([^\s:<>]+@[^\s:<>]+\.[^\s:<>]+)\s*$/);
      if (bare) out.email = bare[1];
    }
    // https://2fa.live   spaced-secret   Year : 2024 ...
    if (!out.token) {
      const live = line.match(
        /https?:\/\/2fa\.live\/?\s+((?:[A-Za-z2-7]{2,8}(?:\s+|$)){3,16})(?=\s*(?:Year|Number|Location|Password|Email)\b|$)/i
      );
      if (live) {
        const secret = live[1].trim().replace(/\s+/g, ' ').replace(/\s*(Year|Number|Location)\s*$/i, '').trim();
        if (isLikely2FA(secret)) {
          out.token = secret.replace(/\s+/g, '');
          if (!out.extra) out.extra = 'https://2fa.live/' + out.token.toUpperCase();
        }
      }
    }
    for (const [key, re] of patterns) {
      const m = line.match(re);
      if (m && m[1] && !out[key]) {
        let val = m[1].trim();
        if (!val || /^[:\-–]*$/.test(val)) {
          if (lines[i + 1] && !/^[A-Za-z0-9].*[=:]/.test(lines[i + 1])) val = lines[i + 1].trim();
        }
        val = val
          .replace(
            /\s+(?:User\s*name|Username|User\s*id|Login|User|Password|Pass|Pwd|E-?mail|Mail|Token|Cookie|2\s*FA\s*\/\s*Token|2\s*FA|2FA|Phone|Mobile|Uid|Year|Number|Location)\s*[:=].*$/i,
            ''
          )
          .trim();
        if (key !== 'token' && key !== 'location') val = val.replace(/\s*[|]\s*.*$/, '').trim();
        // Number / Location often share a line: stop location before next label
        if (key === 'location') val = val.replace(/\s+(?:Year|Number|Password)\s*[:=].*$/i, '').trim();
        if (key === 'phone') val = val.replace(/\s+(?:Year|Location|Password)\s*[:=].*$/i, '').trim();
        val = restoreUrls(val, urls);
        if (val) out[key] = stripQuotes(val);
      }
    }
  }
  // Whole-blob fallbacks for single-line supplier dumps
  const flat = restoreUrls(protectedText, urls);
  if (!out.email) {
    const em = flat.match(/\b([^\s@<>]+@[^\s@<>]+\.[^\s@<>]+)\b/);
    if (em) out.email = em[1];
  }
  if (!out.token) {
    const live2 = flat.match(
      /https?:\/\/2fa\.live\/?\s+((?:[A-Za-z2-7]{2,8}(?:\s+|$)){3,16})(?=\s*(?:Year|Number|Location|Password|Email)\b|$)/i
    );
    if (live2) {
      const secret = live2[1].trim().replace(/\s+/g, ' ').replace(/\s*(Year|Number|Location)\s*$/i, '').trim();
      if (isLikely2FA(secret)) {
        out.token = secret.replace(/\s+/g, '');
        if (!out.extra) out.extra = 'https://2fa.live/' + out.token.toUpperCase();
      }
    }
  }
  if (!out.phone) {
    const ph = flat.match(/\bNumber\s*[:=\-]\s*(\(?\d{3}\)?[\s.-]?\d{3}[\s.-]?\d{4})\b/i);
    if (ph) out.phone = ph[1];
  }
  if (!out.year) {
    const yr = flat.match(/\bYear\s*[:=\-]\s*(\d{4})\b/i);
    if (yr) out.year = yr[1];
  }
  if (!out.location) {
    const loc = flat.match(/\bLocation\s*[:=\-]\s*([^\n]+?)(?=\s+(?:Year|Number|Password)\s*[:=]|$)/i);
    if (loc) out.location = loc[1].trim();
  }
  if (!out.plan) {
    const pl = flat.match(/\bPlan\s*[:=\-]\s*([^\n]+?)(?=\s+(?:Auto\s*Renew|Expire|Days?\s*Left|Password|Email|Username)\s*[:=]|$)/i);
    if (pl) out.plan = pl[1].trim();
  }
  if (!out.autorenew) {
    const ar = flat.match(/\bAuto\s*Renew\s*[:=\-]\s*(\S+)/i);
    if (ar) out.autorenew = ar[1].trim();
  }
  if (!out.expiredate) {
    const ed = flat.match(/\bExpire\s*Date\s*[:=\-]\s*(\S+)/i);
    if (ed) out.expiredate = ed[1].trim();
  }
  if (!out.daysleft) {
    const dl = flat.match(/\bDays?\s*Left\s*[:=\-]\s*(\d+)/i);
    if (dl) out.daysleft = dl[1].trim();
  }
  return out;
}


/**
 * Facebook / Meta style: email:uid:password:2FA:cookieblob
 * Product description "Username:password:2fa" must not remap these parts.
 */

/**
 * Google Voice / Gmail style supplier block:
 * email
 * Password : xxx
 * https://2fa.live   spaced secret   Year : 2020   Number : (818) …
 * Location : (818) California
 */
function parseGoogleVoiceStyle(text) {
  const raw = clean(text);
  if (!raw || raw.length < 12) return null;
  if (!/@/.test(raw)) return null;
  // Need password + (2fa.live or Year/Number labels)
  if (!/password\s*[:=]/i.test(raw)) return null;
  if (!/2fa\.live/i.test(raw) && !/\bYear\s*[:=]/i.test(raw) && !/\bNumber\s*[:=]/i.test(raw)) return null;

  const f = {};
  const emailM = raw.match(/\b([A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,})\b/);
  if (emailM) f.email = emailM[1];

  const passM = raw.match(/\bPassword\s*[:=\-]\s*([^\s\n]+)/i);
  if (passM && !isUrl(passM[1]) && !isEmail(passM[1])) f.password = passM[1].trim();

  // 2FA after 2fa.live
  const live = raw.match(
    /https?:\/\/2fa\.live\/?\s+((?:[A-Za-z2-7]{2,8}(?:\s+|$)){3,20})/i
  );
  if (live) {
    const secret = live[1].replace(/\s*(Year|Number|Location)\s*$/i, '').replace(/\s+/g, '').trim();
    if (secret.length >= 12) f.token = secret.toUpperCase();
  }
  if (!f.token) {
    const spaced = raw.replace(/[\n\r]+/g, ' ').match(/\b((?:[A-Za-z2-7]{3,5}\s+){4,}[A-Za-z2-7]{2,8})\b/i);
    if (spaced) {
      const c = spaced[1].replace(/\s+/g, '');
      if (c.length >= 12 && c.length <= 64) f.token = c.toUpperCase();
    }
  }

  const yr = raw.match(/\bYear\s*[:=\-]\s*(\d{4})\b/i);
  if (yr) f.year = yr[1];

  const num = raw.match(/\bNumber\s*[:=\-]\s*(\(?\d{3}\)?[\s.\-]?\d{3}[\s.\-]?\d{4})\b/i);
  if (num) f.phone = num[1].replace(/\s+/g, ' ').trim();

  const loc = raw.match(/\bLocation\s*[:=\-]\s*(\(?\d{3}\)?\s*[A-Za-z][A-Za-z\s.,]{1,40})/i);
  if (loc) f.location = loc[1].replace(/\s+/g, ' ').trim();

  if (!f.email || !f.password) return null;
  // Never carry Extra for this shape — all fields extracted
  return f;
}

function parseEmailUidPass2faCookie(text) {
  const raw = clean(text);
  if (!raw || raw.length < 20) return null;
  if (!/@/.test(raw)) return null;
  const hasCookie = /(?:datr|c_user|xs|sb|fr)\s*=/i.test(raw);
  const looksFour = /^[^\s:]+:\d{8,}:[^:\s]{4,}:[A-Za-z0-9]{12,}/.test(raw.replace(/\s+/g, ' ').trim());
  if (!hasCookie && !looksFour) return null;
  const { text: prot, urls } = protectUrls(raw.replace(/\n/g, ' '));
  let head = prot;
  const idx = prot.search(/(?:datr|c_user|sb|xs|fr)\s*=/i);
  if (idx > 0) {
    head = prot.slice(0, idx).replace(/:+$/, '');
  }
  const bits = head.split(':').map((x) => stripQuotes(restoreUrls(x, urls))).filter(Boolean);
  if (bits.length < 3) return null;
  if (!isEmail(bits[0])) return null;
  const f = { email: bits[0] };
  if (bits[1]) f.username = bits[1];
  if (bits[2]) f.password = bits[2];
  if (bits[3]) {
    const tok = String(bits[3]).replace(/\s+/g, '');
    if (
      isLikely2FA(tok) ||
      (/^[A-Za-z0-9]{12,64}$/.test(tok) && !isCookieBlob(tok) && !isEmail(tok) && !isUrl(tok))
    ) {
      f.token = tok;
    }
  }
  if (!f.password || !f.email) return null;
  // Never leave password === username
  if (f.password && f.username && String(f.password) === String(f.username) && bits[2] && bits[2] !== f.username) {
    f.password = bits[2];
  }
  if (f.password && f.username && String(f.password) === String(f.username)) return null;
  return f;
}

function applyHint(parts, hintKeys) {
  if (parts && parts.length >= 3 && isEmail(parts[0]) && /^\d{8,}$/.test(String(parts[1] || ''))) {
    const fb = parseEmailUidPass2faCookie(parts.join(':'));
    if (fb && fb.password) return fb;
  }

  const f = {};
  if (!parts || !parts.length || !hintKeys || !hintKeys.length) return null;
  const extras = [];
  const n = Math.min(parts.length, hintKeys.length);
  for (let i = 0; i < n; i++) {
    const key = hintKeys[i];
    const val = parts[i];
    if (!key || val == null || val === '') continue;
    if (key === 'extra') {
      extras.push(val);
      continue;
    }
    if (key === 'cookie') {
      extras.push(val);
      continue;
    }
    if (isUrl(val) && (key === 'password' || key === 'token' || key === 'email_password')) {
      extras.push(val);
      continue;
    }
    if (isCookieBlob(val) && key !== 'token') {
      extras.push(val);
      continue;
    }
    if (isEmail(val) && key !== 'email') {
      if (!f.email) f.email = val;
      else extras.push(val);
      continue;
    }
    // Hint says Email but value has no @ → treat as Username (IG user:pass:2fa)
    if (key === 'email' && !isEmail(val)) {
      if (isLikely2FA(val)) {
        if (!f.token) f.token = val;
        else extras.push(val);
      } else if (!f.username) f.username = val;
      else extras.push(val);
      continue;
    }
    // Hint says Username but value is a real email
    if (key === 'username' && isEmail(val)) {
      if (!f.email) f.email = val;
      else extras.push(val);
      continue;
    }
    if (isLikely2FA(val) && key !== 'token') {
      if (!f.token) f.token = val;
      else extras.push(val);
      continue;
    }
    f[key] = val;
  }
  if (parts.length > hintKeys.length) extras.push(...parts.slice(hintKeys.length));
  if (extras.length) f.extra = [f.extra, extras.join(' | ')].filter(Boolean).join(' | ');
  return f;
}

function mergePreferContent(parts, hinted) {
  const c = fieldsFromClassified(parts || []);
  const f = { ...(hinted || {}) };
  // Drop hinted "email" that is not a real email address
  if (f.email && !isEmail(f.email)) {
    if (!f.username) f.username = f.email;
    delete f.email;
  }
  if (c.email && isEmail(c.email)) f.email = c.email;
  if (c.username) f.username = c.username;
  // Pure digit passwords are valid (VPN / app accounts). Only skip digits when
  // we already have a better non-digit password from labels.
  if (c.password && !isEmail(c.password) && !isLikely2FA(c.password)) {
    if (!f.password) f.password = c.password;
    else if (!/^\d{8,}$/.test(String(c.password)) && /^\d{8,}$/.test(String(f.password))) {
      f.password = c.password;
    }
  }
  if (c.token) f.token = c.token;
  if (c.phone && !f.phone) f.phone = c.phone;
  if (f.email_password && isEmail(f.email_password)) {
    if (!f.email) f.email = f.email_password;
    delete f.email_password;
  }
  // "Email Password" hint wrongly applied to a username (no @) → Username
  if (f.email_password && !isEmail(f.email_password) && !isLikely2FA(f.email_password)) {
    if (!f.username) f.username = f.email_password;
    delete f.email_password;
  }
  if (f.email_password && isLikely2FA(f.email_password)) {
    if (!f.token) f.token = f.email_password;
    delete f.email_password;
  }
  if (f.password && isLikely2FA(f.password) && !f.token) {
    f.token = f.password;
    if (c.password && c.password !== f.password) f.password = c.password;
    else delete f.password;
  }
  // Keep labeled digit passwords; only replace if classified found a clearly better secret
  if (
    f.password &&
    /^\d{8,}$/.test(String(f.password)) &&
    c.password &&
    c.password !== f.password &&
    !/^\d{8,}$/.test(String(c.password)) &&
    !isEmail(c.password) &&
    !isLikely2FA(c.password)
  ) {
    f.password = c.password;
  }
  const used = new Set(
    [f.email, f.username, f.password, f.token, f.email_password, f.phone]
      .filter(Boolean)
      .map((x) => String(x).replace(/\s+/g, '').toLowerCase())
  );
  const leftover = [];
  for (const p of parts || []) {
    const key = String(p || '').replace(/\s+/g, '').toLowerCase();
    if (!key || used.has(key)) continue;
    leftover.push(String(p).trim());
  }
  if (leftover.length) f.extra = leftover.join(' | ');
  else if (c.extra && !f.extra) f.extra = c.extra;
  return f;
}

function flattenJsonObject(obj, depth) {
  if (!obj || typeof obj !== 'object' || depth > 3) return {};
  const f = {};
  const extras = [];
  const walk = (o, d) => {
    if (!o || typeof o !== 'object' || d > 3) return;
    for (const [k, v] of Object.entries(o)) {
      if (v == null) continue;
      const key = String(k).toLowerCase();
      if (typeof v === 'object' && !Array.isArray(v)) {
        walk(v, d + 1);
        continue;
      }
      const val = Array.isArray(v) ? v.map(String).join(' | ') : String(v);
      if (/^(user(name)?|login|uid|id|account)$/.test(key) && !f.username) f.username = val;
      else if (/^(pass(word)?|pwd)$/.test(key) && !f.password) f.password = val;
      else if (/^(email|mail)$/.test(key) && !f.email) f.email = val;
      else if (/email.?pass/.test(key) && !f.email_password) f.email_password = val;
      else if (/^(token|cookie|secret|2fa|twofa|otp)$/.test(key) && !f.token) f.token = val;
      else if (/^(phone|mobile)$/.test(key) && !f.phone) f.phone = val;
      else if (/^(url|link|profile|extra)$/.test(key)) extras.push(val);
      else if (isEmail(val) && !f.email) f.email = val;
      else if (isUrl(val)) extras.push(val);
    }
  };
  walk(obj, 0);
  if (extras.length) f.extra = extras.join(' | ');
  return f;
}

function splitMultipleAccounts(text) {
  const t = clean(text);
  if (!t) return [t];

  const blank = t.split(/\n\s*\n|\n-{3,}\n|\n={3,}\n/).map(clean).filter(Boolean);
  if (blank.length > 1) return blank;

  // Repeated email:pass on new lines
  const lines = t.split(/\n+/).map(clean).filter(Boolean);
  const emailPassLines = lines.filter((ln) => /^[^\s@]+@[^\s@]+\.[^\s@]+\s*[:|;]\s*\S+/.test(ln));
  if (emailPassLines.length >= 2 && emailPassLines.length === lines.length) return lines;

  // Two emails in one blob → split before second email
  const emails = [...t.matchAll(/[^\s@<>]+@[^\s@<>]+\.[^\s@<>]+/g)];
  if (emails.length >= 2) {
    const chunks = [];
    let last = 0;
    for (let i = 0; i < emails.length; i++) {
      const start = emails[i].index;
      if (i === 0) continue;
      const slice = t.slice(last, start).trim();
      if (slice) chunks.push(slice);
      last = start;
    }
    const tail = t.slice(last).trim();
    if (tail) chunks.push(tail);
    if (chunks.length >= 2) return chunks;
  }

  // Two long digit uids — ignore IDs inside URLs or cookie strings
  const uids = [...t.matchAll(/\b\d{10,}\b/g)].filter((m) => {
    const before = t.slice(Math.max(0, m.index - 40), m.index);
    if (/https?:\/\/\S*$/i.test(before) || /profile\.php\?id=$/i.test(before)) return false;
    if (/(?:c_user|xs|datr|sb|fr|pass)=\s*$/i.test(before)) return false;
    if (/c_user=|xs=|datr=/i.test(t)) return false; // whole blob is a cookie — do not split
    return true;
  });
  if (uids.length >= 2 && uids[1].index - uids[0].index > 8) {
    const mid = uids[1].index;
    const a = t.slice(0, mid).trim();
    const b = t.slice(mid).trim();
    if (a.length > 8 && b.length > 8) return [a, b];
  }
  return [t];
}

function cleanupFields(f) {

  // Username duplicated as Password (common FB mis-map)
  if (f.password && f.username && String(f.password) === String(f.username)) {
    delete f.password;
  }
  if (!f.password && f.email_password && !isEmail(f.email_password) && !isLikely2FA(f.email_password)) {
    f.password = f.email_password;
    delete f.email_password;
  }

  // Facebook UIDs (often 15+ digits, no email context) must not show as Number.
  // Real phone numbers (10–11 digits) with email stay as Number.
  if (f.phone && /^\d{8,16}$/.test(String(f.phone).trim())) {
    const d = String(f.phone).trim();
    const looksUid = d.length >= 14 || (!f.email && !f.email_password && !f.recovery);
    if (looksUid) {
      if (!f.username) f.username = f.phone;
      delete f.phone;
    }
  }
  // Cookie blob wrongly stored as password → move to token
  if (f.password && isCookieBlob(f.password)) {
    f.token = f.token ? f.password + ' | ' + f.token : f.password;
    delete f.password;
  }
  // Product title noise (HMA VPN 30 days etc.)
  if (f.username && /^(HMA|VPN|NORD|EXPRESS|SURFSHARK|PROTON)$/i.test(String(f.username).trim())) delete f.username;
  if (f.password && /^(HMA|VPN|NORD|EXPRESS|30\s*days?|1\s*month)$/i.test(String(f.password).trim())) delete f.password;

  if (f.password && /^(https?)$/i.test(String(f.password).trim())) delete f.password;
  if (f.username && /^(https?)$/i.test(String(f.username).trim())) delete f.username;
  // Never keep field labels as values (bug: "Username: Email")
  const LABEL_WORDS = /^(email|e-?mail|username|user\s*name|user|password|pass|pwd|login|uid|2fa|token|extra|phone|number|year|location|plan|autorenew|expiredate|daysleft)$/i;
  if (f.username && LABEL_WORDS.test(String(f.username).trim())) delete f.username;
  if (f.password && LABEL_WORDS.test(String(f.password).trim())) delete f.password;
  if (f.email && LABEL_WORDS.test(String(f.email).trim())) delete f.email;
  // user:pass:email:mailpass — last chunk is often an email password, not 2FA
  if (f.email && f.password && f.token && !f.email_password) {
    const tok = String(f.token).replace(/\s+/g, '');
    if (tok.length <= 20 && /[a-z]/.test(tok) && /[A-Z]/.test(tok)) {
      f.email_password = String(f.token);
      delete f.token;
    }
  }
  if (f.email && f.password && f.username && f.extra && !f.email_password && !f.token) {
    const ex = String(f.extra).trim();
    if (ex && !/\s/.test(ex) && ex.length <= 32 && !isUrl(ex) && !isEmail(ex)) {
      f.email_password = ex;
      delete f.extra;
    }
  }
  // "Plan = 24 Month" must never sit in Username
  if (f.username && /^\s*plan\s*=/i.test(String(f.username))) {
    if (!f.plan) f.plan = String(f.username).replace(/^\s*plan\s*=\s*/i, '').trim();
    delete f.username;
  }
  if (f.username && /plan\s*=\s*\d+/i.test(String(f.username))) {
    if (!f.plan) f.plan = String(f.username).replace(/^.*plan\s*=\s*/i, '').trim();
    delete f.username;
  }
  // Pull VPN meta out of Extra: "AutoRenew = true ExpireDate = 2027-10-02 DaysLeft = 454"
  if (f.extra) {
    const ex = String(f.extra);
    if (!f.plan) {
      const m = ex.match(/\bPlan\s*[:=\-]\s*([^|]+?)(?=\s+(?:Auto\s*Renew|Expire|Days?\s*Left)\b|$)/i);
      if (m) f.plan = m[1].trim();
    }
    if (!f.autorenew) {
      const m = ex.match(/\bAuto\s*Renew\s*[:=\-]\s*(\S+)/i);
      if (m) f.autorenew = m[1].trim();
    }
    if (!f.expiredate) {
      const m = ex.match(/\bExpire\s*Date\s*[:=\-]\s*(\S+)/i);
      if (m) f.expiredate = m[1].trim();
    }
    if (!f.daysleft) {
      const m = ex.match(/\bDays?\s*Left\s*[:=\-]\s*(\d+)/i);
      if (m) f.daysleft = m[1].trim();
    }
    if (f.plan || f.autorenew || f.expiredate || f.daysleft) {
      let left = ex
        .replace(/\bPlan\s*[:=\-]\s*[^|]*?(?=\s+(?:Auto\s*Renew|Expire|Days?\s*Left)\b|$)/ig, '')
        .replace(/\bAuto\s*Renew\s*[:=\-]\s*\S+/ig, '')
        .replace(/\bExpire\s*Date\s*[:=\-]\s*\S+/ig, '')
        .replace(/\bDays?\s*Left\s*[:=\-]\s*\d+/ig, '')
        .replace(/[|]+/g, ' ')
        .replace(/\s+/g, ' ')
        .trim();
      if (!left) delete f.extra;
      else f.extra = left;
    }
  }
  // If "email" was wrongly put in username and real email is in extra, recover
  if (!f.email && f.extra) {
    const emFromExtra = String(f.extra).match(/\b([^\s|<>]+@[^\s|<>]+\.[^\s|<>]+)\b/);
    if (emFromExtra) f.email = emFromExtra[1];
  }
  // "Email" without @ is not an email (e.g. mist.2z2381oxx774997) → Username
  if (f.email && !isEmail(f.email)) {
    if (!f.username) f.username = f.email;
    else if (String(f.username).toLowerCase() !== String(f.email).toLowerCase()) {
      f.extra = [f.email, f.extra].filter(Boolean).join(' | ');
    }
    delete f.email;
  }
  const em = String(f.email || '').trim().toLowerCase();
  const un = String(f.username || '').trim().toLowerCase();
  // Same login printed twice (email used as username)
  if (em && un && em === un) delete f.username;
  // Username field is actually an email and Email is empty → keep Email only
  if (!em && un && isEmail(f.username)) {
    f.email = f.username;
    delete f.username;
  }
  // Recover pure-digit password stuck only in Extra (VPN accounts)
  if (!f.password && f.extra) {
    const onlyDigits = String(f.extra).trim().match(/^(\d{6,16})$/);
    if (onlyDigits) {
      f.password = onlyDigits[1];
      delete f.extra;
    }
  }
  // Drop Extra that only repeats email/password we already show
  if (f.extra) {
    let ex = String(f.extra);
    if (f.email) {
      const re = new RegExp(String(f.email).replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi');
      ex = ex.replace(re, '');
    }
    if (f.password) {
      const re = new RegExp(String(f.password).replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi');
      ex = ex.replace(re, '');
    }
    if (f.token) {
      const tok = String(f.token).replace(/\s+/g, '');
      ex = ex.replace(new RegExp(tok.split('').join('\\s*'), 'i'), '');
    }
    ex = ex
      .replace(/\b(password|pass|email|username|user|2fa|token)\b/gi, '')
      .replace(/[|]+/g, ' ')
      .replace(/\s+/g, ' ')
      .trim();
    if (!ex) delete f.extra;
    else f.extra = ex;
  }
  if (f.password && /^\/\//.test(String(f.password).trim())) {
    f.extra = ['https:' + f.password, f.extra].filter(Boolean).join(' | ');
    delete f.password;
  }
  if (f.token && /^\/\//.test(String(f.token).trim())) f.token = 'https:' + f.token;
  if (f.token && isLikely2FA(f.token)) f.token = String(f.token).replace(/\s+/g, '');

  // Username same as email is redundant
  if (f.username && f.email && String(f.username).toLowerCase() === String(f.email).toLowerCase()) {
    delete f.username;
  }

  // Username same as email is redundant
  if (f.username && f.email && String(f.username).toLowerCase() === String(f.email).toLowerCase()) {
    delete f.username;
  }

  // Drop Extra if it only repeats fields we already show
  if (f.extra) {
    let ex = String(f.extra);
    ex = ex.replace(/https?:\/\/2fa\.live\/?/gi, ' ');
    ex = ex.replace(/https?:\/\/\.?live\/?/gi, ' ');
    if (f.year) ex = ex.replace(new RegExp(String(f.year), 'g'), ' ');
    if (f.phone) {
      const d = String(f.phone).replace(/\D/g, '');
      if (d.length >= 10) {
        ex = ex.replace(/\(?\d{3}\)?[\s.\-]*\d{3}[\s.\-]*\d{4}/g, ' ');
      }
    }
    ex = ex.replace(/\b(Year|Number|Location|Password|Email|Username)\s*[:=\-]?/gi, ' ');
    if (f.token) {
      const c = String(f.token).replace(/\s+/g, '');
      if (c.length >= 8) {
        try { ex = ex.replace(new RegExp(c, 'i'), ' '); } catch (_) {}
      }
    }
    if (f.password) {
      try {
        const pw = String(f.password).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        ex = ex.replace(new RegExp(pw, 'gi'), ' ');
      } catch (_) {}
    }
    if (f.email) {
      try {
        const em = String(f.email).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        ex = ex.replace(new RegExp(em, 'gi'), ' ');
      } catch (_) {}
    }
    ex = ex.replace(/[|:;,.\-]+/g, ' ').replace(/\s+/g, ' ').trim();
    if (!ex || ex.length < 4) delete f.extra;
    else f.extra = ex;
  }

  return f;
}

function fieldsToText(f) {
  f = cleanupFields({ ...f });
  f = polishLabeled(f, [f.username, f.email, f.password, f.token, f.extra, f.year, f.phone, f.location].filter(Boolean).join('\n'));
  // Re-scrub after polishLabeled (it can reintroduce noise)
  f = cleanupFields({ ...f });
  const lines = [];
  // Email-first when we have an email (Google Voice / Gmail style); else Number-first for pure phone logs
  if (f.email) {
    lines.push('Email: ' + f.email);
    if (f.username && String(f.username).toLowerCase() !== String(f.email).toLowerCase()) {
      lines.push('Username: ' + f.username);
    }
    if (f.password) lines.push('Password: ' + f.password);
    if (f.email_password) lines.push('Email Password: ' + f.email_password);
    if (f.recovery) lines.push('Recovery Email: ' + f.recovery);
  } else if (f.phone) {
    lines.push('Number: ' + f.phone);
    if (f.password) lines.push('Password: ' + f.password);
    if (f.username && f.username !== f.phone) lines.push('Username: ' + f.username);
  } else {
    if (f.username) lines.push('Username: ' + f.username);
    if (f.password) lines.push('Password: ' + f.password);
    if (f.email_password) lines.push('Email Password: ' + f.email_password);
  }
  if (f.token) {
    let tok = String(f.token);
    const cookie = isCookieBlob(tok) || /c_user\s*=|xs\s*=|datr\s*=|sb\s*=|wL_cbv=/i.test(tok) || (tok.length > 80 && /;/.test(tok) && /=/.test(tok));
    if (cookie) {
      lines.push('Cookie / Token: ' + String(f.token).replace(/\n/g, ' ').trim());
    } else {
      lines.push('2FA / Token: ' + pretty2fa(tok));
    }
  }
  if (f.phone && f.email) lines.push('Number: ' + f.phone);
  if (f.year) lines.push('Year: ' + f.year);
  if (f.location) lines.push('Location: ' + f.location);
  if (f.plan) lines.push('Plan: ' + String(f.plan).replace(/^\s*plan\s*=\s*/i, '').trim());
  if (f.autorenew) lines.push('AutoRenew: ' + String(f.autorenew).replace(/^\s*auto\s*renew\s*=\s*/i, '').trim());
  if (f.expiredate) lines.push('ExpireDate: ' + String(f.expiredate).replace(/^\s*expire\s*date\s*=\s*/i, '').trim());
  if (f.daysleft) lines.push('DaysLeft: ' + String(f.daysleft).replace(/^\s*days?\s*left\s*=\s*/i, '').trim());
  if (f.extra) {
    let extra = String(f.extra).trim();
    // Never dump a bare 2fa.live link when token is already on 2FA line
    if (f.token && /2fa\.live/i.test(extra) && !/[A-Za-z0-9]{8,}/.test(extra.replace(/https?:\/\/2fa\.live\/?/gi, ''))) {
      extra = '';
    }
    if (extra && extra.length >= 4) lines.push('Extra: ' + extra);
  }
  return lines.join('\n');
}

function repairBrokenFormatted(text) {
  let s = clean(text);
  if (!s) return '';
  if (!/\/\/[a-z0-9.-]+/i.test(s) && !/\bhttps\s*$/im.test(s) && !/Username:.*\bpassword\b/i.test(s)) {
    return '';
  }
  const flat = s.replace(/[\n\r]+/g, ' ');
  const secretMatches = flat.match(/\b(?:[A-Z2-7]{2,8}(?:\s+[A-Z0-9]{2,8}){2,})\b/g) || [];
  let token = '';
  for (const m of secretMatches) {
    const c = m.replace(/\s+/g, '');
    if (c.length >= 12 && c.length <= 64 && c.length > token.length) token = c;
  }
  const uidM = s.match(/\b(?:uid\s*number\s*)?(\d{10,})\b/i);
  const username = uidM ? uidM[1] : '';
  let password = '';
  const passM = s.match(/\bpassword\s*[:=\-]?\s*([A-Za-z0-9._@!#$%*-]+)/i);
  if (passM && !/^(https?)$/i.test(passM[1]) && !/^\/\//.test(passM[1])) password = passM[1];
  const links = [];
  const fbIds = [...s.matchAll(/profile\.php\?\s*id=(\d+)/gi)].map((x) => x[1]);
  for (const id of fbIds) {
    const u = 'https://www.facebook.com/profile.php?id=' + id;
    if (!links.includes(u)) links.push(u);
  }
  if (/2fa\.live/i.test(s)) {
    const u = token ? 'https://2fa.live/' + token : 'https://2fa.live/';
    if (!links.some((l) => l.includes('2fa.live'))) links.unshift(u);
  }
  if (!username && !password && !token) return '';
  const lines = [];
  if (username) lines.push('Username: ' + username);
  if (password) lines.push('Password: ' + password);
  if (token) lines.push('2FA / Token: ' + token);
  if (links.length) lines.push('Extra: ' + links.join(' | '));
  return lines.join('\n');
}


function parseLooseSocialLog(text) {
  const raw = clean(text);
  if (!raw || !/@/.test(raw)) return '';
  const emailM = raw.match(/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/);
  if (!emailM) return '';
  const { text: prot, urls } = protectUrls(raw);
  const fa = prot.match(/\b(?:[A-Za-z0-9]{4}\s+){5,}[A-Za-z0-9]{4}\b/);
  let token = '';
  let work = prot;
  if (fa) {
    token = fa[0].replace(/\s+/g, '').toUpperCase();
    work = work.replace(fa[0], ' ');
  }
  work = work.replace(emailM[0], ' ');
  const uidM = work.match(/\b(\d{10,})\b/);
  const uid = uidM ? uidM[1] : '';
  if (uid) work = work.replace(uid, ' ');
  work = restoreUrls(work, urls).replace(/https?:\/\/\S+/gi, ' ');
  const leftovers = work.split(/\s+/).map(stripQuotes).filter(Boolean);
  let password = '';
  const extraBits = [];
  for (const tok of leftovers) {
    if (!password && tok.length >= 4 && tok.length <= 48 && !isEmail(tok) && !isUrl(tok) && !/^\d{10,}$/.test(tok)) {
      password = tok;
    } else extraBits.push(tok);
  }
  if (!password && !token) return '';
  const extra = [extraBits.join(' '), uid, urls[0]].filter(Boolean).join(' | ');
  return fieldsToText({
    email: emailM[0],
    password: password || undefined,
    token: token || undefined,
    extra: extra || undefined
  });
}

function extractPhonePasswordLine(text) {
  const s = clean(text).replace(/^\s*\d+\.\s*/, '');
  const m = s.match(/^\s*(\+?\d[\d\s().-]{6,22}\d)\s*[\/|:]\s*(.+)$/);
  if (!m) return null;
  const digits = m[1].replace(/\D/g, '');
  if (digits.length < 10 || digits.length > 15) return null;
  // Pure digit IDs (Facebook UID etc.) are NOT phone numbers — need +, space, dash, or ()
  const phoneRaw = m[1].trim();
  if (/^\d{8,16}$/.test(phoneRaw)) return null;
  if (!/[+()\-\s]/.test(phoneRaw) && !/^\+/.test(phoneRaw)) return null;
  if (/\b2FA\s*\/\s*Token:/i.test(s) && /\bUsername:/i.test(s)) return null;
  const phone = phoneRaw.replace(/\s+/g, ' ').trim();
  const rest = String(m[2] || '').trim();
  if (!rest || isUrl(rest)) return null;
  const parts = rest.split(/\s*[\/|]\s*/).map((p) => p.trim()).filter(Boolean);
  const emailIdxs = parts.map((p, i) => (isEmail(p) ? i : -1)).filter((i) => i >= 0);
  if (emailIdxs.length) {
    const emailAt = emailIdxs[0];
    const password = parts.slice(0, emailAt).join('/') || undefined;
    if (password && isEmail(password)) return null;
    const email = parts[emailAt];
    const after = parts.slice(emailAt + 1);
    let email_password;
    let recovery;
    if (after.length && isEmail(after[after.length - 1]) && after.length >= 2) {
      recovery = after[after.length - 1];
      email_password = after.slice(0, -1).join('/') || undefined;
    } else if (after.length === 1 && isEmail(after[0])) {
      recovery = after[0];
    } else if (after.length) {
      const lastEmail = after.findIndex((p) => isEmail(p));
      if (lastEmail >= 0) {
        recovery = after[lastEmail];
        email_password = after.filter((_, i) => i !== lastEmail).join('/') || undefined;
      } else {
        email_password = after.join('/');
      }
    }
    return {
      phone,
      password,
      email,
      email_password,
      recovery
    };
  }
  if (isEmail(rest)) return { phone, email: rest };
  return { phone, password: rest };
}


/**
 * WhatsApp / number-first products: rebuild Number/Password/Email/Email Password/Recovery
 * when supplier order is messy (email-first, Extra dump, leading +, phone at end).
 */
function repairWhatsAppShape(f, formatHint) {
  const hintStr = Array.isArray(formatHint)
    ? formatHint.join('/')
    : String(formatHint || '');
  const wantsPhoneFirst =
    /number\s*[\/|]\s*password/i.test(hintStr) ||
    (Array.isArray(formatHint) && formatHint[0] === 'phone') ||
    /whatsapp/i.test(hintStr);

  // Pull candidates from all string fields
  const bag = [f.phone, f.username, f.email, f.password, f.email_password, f.recovery, f.token, f.extra]
    .filter(Boolean)
    .map((x) => String(x))
    .join(' ');

  const emails = [...bag.matchAll(/[^\s|<>\/:]+@[^\s|<>\/:]+\.[^\s|<>\/:]+/gi)].map((m) => m[0]);
  const phones = [];
  for (const m of bag.matchAll(/\+?\d[\d\s().-]{8,16}\d|\b\d{10,15}\b/g)) {
    const digs = m[0].replace(/\D/g, '');
    if (digs.length >= 10 && digs.length <= 15 && digs.length < 14) phones.push(digs);
  }
  // Unique preserve order
  const uniq = (arr) => {
    const s = new Set();
    const out = [];
    for (const x of arr) {
      const k = x.toLowerCase();
      if (s.has(k)) continue;
      s.add(k);
      out.push(x);
    }
    return out;
  };
  const ems = uniq(emails);
  const phs = uniq(phones);

  // Password candidates: known Importance style or current password if not email/phone
  let password = f.password && !isEmail(f.password) && !/^\+?\d+$/.test(String(f.password).replace(/\D/g,''))
    ? String(f.password).trim()
    : '';
  if (!password) {
    const imp = bag.match(/\/?Importance[^|\s]*/i);
    if (imp) password = imp[0].startsWith('/') ? imp[0] : '/' + imp[0];
  }
  // Email password: short alphanumeric not email/phone
  let email_password = f.email_password ? String(f.email_password).trim() : '';
  if (!email_password) {
    const parts = bag.split(/[\s|\/:]+/).map((x) => x.trim()).filter(Boolean);
    for (const part of parts) {
      if (isEmail(part)) continue;
      if (/^(email|password|pass|pwd|extra|number|recovery|username|user)$/i.test(part)) continue;
      if (/^\+?\d{10,15}$/.test(part.replace(/\D/g, '')) && part.replace(/\D/g,'').length >= 10) continue;
      if (/importance/i.test(part)) continue;
      if (/^https?:/i.test(part)) continue;
      if (part.length >= 6 && part.length <= 24 && /[a-z]/i.test(part) && !/@/.test(part) && part !== password) {
        email_password = part.replace(/^\//, '');
        break;
      }
    }
  }

  if (!(wantsPhoneFirst || phs.length || (ems.length >= 2 && password))) {
    return f;
  }
  if (!password && !phs.length && ems.length < 1) return f;

  const out = {};
  if (phs.length) out.phone = phs[0];
  if (password) out.password = password;
  if (ems.length) out.email = ems[0];
  if (email_password) out.email_password = email_password;
  if (ems.length >= 2) out.recovery = ems[1];
  // Drop noisy Extra when we successfully rebuilt
  if (out.phone && out.email && out.password) return out;
  return f;
}

export function formatOneAccount(raw, formatHint) {
  let text = stitchUsPhone(clean(raw));
  if (!text) return '';

  const gvShape = parseGoogleVoiceStyle(text);
  if (gvShape && gvShape.email && gvShape.password) {
    return fieldsToText(cleanupFields(gvShape));
  }

  const fbShape = parseEmailUidPass2faCookie(text);
  if (fbShape && fbShape.email && fbShape.password) {
    return fieldsToText(cleanupFields(fbShape));
  }

  // Strip product-title preamble (HMA VPN 30 days, NordVPN Premium, etc.)
  text = text
    .replace(/^(?:[A-Za-z0-9][\w\s.-]{0,30}?\s)?(?:HMA|Nord|Express|Surfshark|Proton|IPVanish|Private\s*Internet)[\w\s.-]{0,40}(?:VPN|Premium|Account)?[\w\s.-]{0,25}\n+/i, '')
    .replace(/^\s*\d+\s*month[s]?\s+[A-Z]{2,10}\s*VPN[^\n]*\n+/i, '');
  // Normalize Code: → token label for VPN
  text = text.replace(/(^|\n)\s*Code\s*[:=\-]\s*/gi, '$12FA / Token: ');
  // Drop leading account index "1. " / "2. " on labeled lines
  text = text.replace(/(^|\n)\s*\d+\.\s+(?=(?:User\s*name|Username|Email|Password|Pass|Code|2\s*FA)\b)/gi, '$1');

  // Facebook cookie delivery: uid:password:cookieblob
  {
    const fb = text.match(/^(\d{8,16})\s*[:|]\s*([^\s:|]{4,64})\s*[:|]\s*(.+)$/s);
    if (fb && (isCookieBlob(fb[3]) || /c_user=|xs=|datr=|sb=/i.test(fb[3]))) {
      return fieldsToText({
        username: fb[1],
        password: fb[2],
        token: fb[3].trim()
      });
    }
  }

  // WhatsApp / phone logs: number/pass/email/emailpass/recovery ( / or : separators)
  {
    const flat = text.replace(/\n+/g, ' ').trim();
    const wa = flat.match(
      /^\s*(\+?\d[\d\s().-]{8,18}\d|\d{10,15})\s*[:|]\s*(.+?)\s*[:|]\s*([^\s:|]+@[^\s:|]+)\s*[:|]\s*(.+)$/
    ) || flat.match(
      /^\s*(\+?\d[\d\s().-]{8,18}\d|\d{10,15})\s*\/\s*(.+?)\s*\/\s*([^\s\/]+@[^\s\/]+)\s*\/\s*(.+)$/
    );
    if (wa) {
      const digs = wa[1].replace(/\D/g, '');
      if (digs.length >= 10 && digs.length <= 15) {
        const password = wa[2].trim();
        const email = wa[3].trim();
        const after = wa[4].split(/\s*[\/|:]\s*/).map((x) => x.trim()).filter(Boolean);
        let email_password, recovery;
        const emailsAfter = after.filter((x) => isEmail(x));
        if (emailsAfter.length) {
          recovery = emailsAfter[emailsAfter.length - 1];
          const rest = after.filter((x) => x !== recovery);
          email_password = rest.join('/') || undefined;
        } else if (after.length) {
          email_password = after.join('/');
        }
        return fieldsToText({
          phone: digs,
          password,
          email,
          email_password,
          recovery
        });
      }
    }
  }


  // Labeled multi-line (VPN / manual): Username/Email + Password + Code
  {
    const lab = extractLabeled(text);
    if ((lab.email || lab.username) && lab.password && (lab.token || /2FA|Code/i.test(text))) {
      if (!lab.token) {
        const cm = text.match(/(?:^|\n)\s*(?:code|2\s*fa)\s*[:=\-]\s*(\S+)/i);
        if (cm) lab.token = cm[1].trim();
      }
      // Drop title-like username
      if (lab.username && /vpn|hma|nord|month|days/i.test(lab.username) && lab.email) delete lab.username;
      return fieldsToText(lab);
    }
  }


  // Email-first WA: email:pass:phone:mailpass:recovery OR email:pass:... with phone later
  {
    const flat = text.replace(/\n+/g, ' ').trim();
    const ef = flat.match(
      /^\s*([^\s:|]+@[^\s:|]+)\s*[:|\/]\s*([^:|\/]+?)\s*[:|\/]\s*(.+)$/
    );
    if (ef && isEmail(ef[1])) {
      const email = ef[1].trim();
      let password = ef[2].trim();
      const rest = ef[3];
      const digs = (rest.match(/\b(\d{10,15})\b/) || rest.match(/\+(\d{10,15})\b/) || [])[1];
      const restEmails = [...rest.matchAll(/[^\s:|\/]+@[^\s:|\/]+/g)].map((m) => m[0]).filter((e) => e.toLowerCase() !== email.toLowerCase());
      const tokens = rest.split(/[\s:|\/]+/).map((x) => x.trim()).filter(Boolean);
      let email_password = tokens.find((t) => !isEmail(t) && !/^\+?\d{10,15}$/.test(t.replace(/\D/g,'')) && !/importance/i.test(t) && t.length >= 6 && t.length <= 24);
      if (digs || restEmails.length) {
        return fieldsToText({
          phone: digs || undefined,
          password,
          email,
          email_password: email_password || undefined,
          recovery: restEmails[0] || undefined
        });
      }
    }
  }

  const fixedUs = repairSplitUsLabeled(text);
  if (fixedUs) return fixedUs;
  if (/^Number\s*:/im.test(text) && /^Password\s*:/im.test(text) && /^Email\s*:/im.test(text)) {
    const labeledEarly = extractLabeled(text);
    if (labeledEarly.phone && labeledEarly.password) return fieldsToText(labeledEarly);
  }
  {
    const extracted = extractPhonePasswordLine(text);
    if (extracted) return fieldsToText(extracted);
    const np = text.match(/^\s*(\+?\d[\d\s().-]{8,16}\d)\s*[\/|:]\s*([^\s\/][^\n]{0,80}?)\s*$/);
    if (np && isPhone(np[1].trim()) && !isUrl(np[2].trim()) && !isEmail(np[2].trim())) {
      return fieldsToText({ phone: np[1].trim(), password: np[2].trim() });
    }
  }
  if (text.split(':').filter(Boolean).length < 3) {
    const loose = parseLooseSocialLog(text);
    if (loose && /^Email:/m.test(loose) && /Password:|2FA \/ Token:/m.test(loose)) return loose;
  }

  // Normalize broken supplier labels: "E -mail", "E-mail", "E mail" → Email
  text = text
    .replace(/(^|\n)\s*E\s*[-–—]?\s*mail\b/gi, '$1Email')
    .replace(/(^|\n)\s*Pass\s*word\b/gi, '$1Password')
    .replace(/(^|\n)\s*User\s*name\b/gi, '$1Username');

  // Fast path: clean supplier "Email: … / Password: …" blocks (incl. digit passwords)
  {
    const em = text.match(/(?:^|\n)\s*e-?mail\s*[:=\-]?\s*([^\s\n]+@[^\s\n]+)/i);
    const pw = text.match(/(?:^|\n)\s*pass(?:word)?\s*[:=\-]?\s*(\S+)/i);
    if (em && pw && !/2fa\.live/i.test(text) && (text.match(/\n/g) || []).length <= 3) {
      const more =
        /(?:^|\n)\s*(?:year|number|location|username|user|2fa|token)\s*[:=\-]/i.test(text);
      if (!more) {
        return fieldsToText({ email: em[1].trim(), password: pw[1].trim() });
      }
    }
  }

  // Fast path: VPN / manual accounts with Plan / AutoRenew / ExpireDate / DaysLeft
  {
    const em = text.match(/(?:^|\n)\s*e-?mail\s*[:=\-]\s*([^\s\n]+@[^\s\n]+)/i);
    const pw = text.match(/(?:^|\n)\s*pass(?:word)?\s*[:=\-]\s*(\S+)/i);
    const plan =
      text.match(/(?:^|\n)\s*(?:username\s*[:=\-]\s*)?plan\s*[:=\-]\s*([^\n]+)/i) ||
      text.match(/\bplan\s*[:=\-]\s*([^|\n]+?)(?=\s+(?:Auto\s*Renew|Expire|Days?\s*Left|Password|Email)\b|$)/i);
    const ar = text.match(/\bauto\s*renew\s*[:=\-]\s*(\S+)/i);
    const ed = text.match(/\bexpire\s*date\s*[:=\-]\s*(\S+)/i);
    const dl = text.match(/\bdays?\s*left\s*[:=\-]\s*(\d+)/i);
    if (em && pw && (plan || ar || ed || dl)) {
      return fieldsToText({
        email: em[1].trim(),
        password: pw[1].trim(),
        plan: plan ? String(plan[1]).replace(/\s+/g, ' ').trim() : undefined,
        autorenew: ar ? ar[1].trim() : undefined,
        expiredate: ed ? ed[1].trim() : undefined,
        daysleft: dl ? dl[1].trim() : undefined
      });
    }
  }

  const labeled = polishLabeled(extractLabeled(text), text);
  const previewParts = splitDelimiters(text.replace(/\n/g, ' ').replace(/\s+/g, ' '), Array.isArray(formatHint) ? formatHint.length : 0);
  const previewClassified = fieldsFromClassified(previewParts || []);
  const labeledMissing =
    (previewClassified.email && !labeled.email) ||
    (previewClassified.token && !labeled.token) ||
    (previewClassified.extra && !labeled.extra && !labeled.token);
  if (!labeledMissing && (labeled.password || labeled.username || labeled.email || labeled.token)) {
    if (labeled.password && /^(https?)$/i.test(labeled.password)) {
      /* keep going */
    } else if (/^(Email|Username|Password|2FA \/ Token|2FA|Phone|Cookie)\s*:/im.test(text)) {
      return fieldsToText(labeled);
    } else {
      return fieldsToText(mergePreferContent(previewParts, labeled));
    }
  }

  if (text.startsWith('{') || text.startsWith('[')) {
    try {
      const j = JSON.parse(text);
      const obj = Array.isArray(j) ? j[0] : j;
      if (obj && typeof obj === 'object') {
        const t = fieldsToText(flattenJsonObject(obj, 0));
        if (t) return t;
      }
    } catch (_) {}
  }

  const expected = Array.isArray(formatHint) ? formatHint.length : 0;
  const parts = splitDelimiters(text.replace(/\n/g, ' ').replace(/\s+/g, ' '), expected);
  if (globalThis.DEBUGFMT) console.log('parts', parts, 'hint', formatHint);
  if (parts && parts.length >= 2) {
    const hinted = applyHint(parts, formatHint);
    if (hinted && (hinted.username || hinted.password || hinted.email || hinted.token || hinted.email_password)) {
      return fieldsToText(mergePreferContent(parts, hinted));
    }
    return fieldsToText(mergePreferContent(parts, null));
  }

  const lines = text.split(/\n+/).map(clean).filter(Boolean);
  if (lines.length >= 2 && !lines[0].includes(' ')) {
    const hinted = applyHint(lines, formatHint);
    if (hinted) return fieldsToText(mergePreferContent(lines, hinted));
    return fieldsToText(mergePreferContent(lines, null));
  }

  const prose = parseProseCredential(text);
  if (prose && prose.length >= 2) return fieldsToText(fieldsFromClassified(prose));

  const spaced = splitSpacedTokens(text);
  if (spaced && spaced.length >= 2) return fieldsToText(fieldsFromClassified(spaced));

  if (isCookieBlob(text)) return fieldsToText({ token: text });

  return text.replace(/[ \t]+\n/g, '\n').trim();
}

export function formatCredentials(details, description) {
  if (details == null) return '';
  if (typeof details === 'object') {
    try {
      details = JSON.stringify(details);
    } catch (_) {
      details = String(details);
    }
  }
  let text = stitchUsPhone(clean(details));
  if (!text) return '';

  const gvLine = parseGoogleVoiceStyle(text);
  if (gvLine && gvLine.email && gvLine.password) {
    return fieldsToText(cleanupFields(gvLine));
  }

  const fbLine = parseEmailUidPass2faCookie(text);
  if (fbLine && fbLine.email && fbLine.password) {
    return fieldsToText(cleanupFields(fbLine));
  }

  const fixedUs = repairSplitUsLabeled(text);
  if (fixedUs) return fixedUs;

  const phoneLine = extractPhonePasswordLine(text);
  if (phoneLine && (phoneLine.password || phoneLine.email)) return fieldsToText(phoneLine);

  const alreadyLabeled = /^(Email|Username|Password|Number|Phone|Email Password|Recovery Email|2FA \/ Token|2FA)\s*:/im.test(text);
  const colonParts = text.split(':').filter(Boolean);
  if (!alreadyLabeled && colonParts.length < 3) {
    const loose = parseLooseSocialLog(text);
    if (loose && /^Email:/m.test(loose) && /Password:/.test(loose)) return loose;
    const repaired = repairBrokenFormatted(text);
    if (repaired && repaired.includes('Password:') && repaired.includes('Email:')) return repaired;
  }

  // Drop product title lines before splitting multi-account blobs
  text = text
    .replace(/^(?:[A-Za-z0-9][\w\s.-]{0,30}?\s)?(?:HMA|Nord|Express|Surfshark|Proton|IPVanish|Private\s*Internet)[\w\s.-]{0,40}(?:VPN|Premium|Account)?[\w\s.-]{0,25}\n+/i, '')
    .replace(/^\s*\d+\s*month[s]?\s+[A-Z]{2,10}\s*VPN[^\n]*\n+/i, '')
    .replace(/(^|\n)\s*Code\s*[:=\-]\s*/gi, '$12FA / Token: ')
    .replace(/(^|\n)\s*\d+\.\s+(?=(?:User\s*name|Username|Email|Password|Pass|Code|2\s*FA)\b)/gi, '$1')
    .trim();

  const hint = parseFormatHint(description);
  const descStr = String(description || '');
  const isWhatsAppLog =
    /whatsapp/i.test(descStr) ||
    /number\s*[\/|]\s*password\s*[\/|]\s*email/i.test(descStr) ||
    (Array.isArray(hint) && hint[0] === 'phone' && hint.includes('email'));
  // WA accounts often include 2 emails (login + recovery) — do NOT split into 2 "accounts"
  let chunks = isWhatsAppLog
    ? [text]
    : splitMultipleAccounts(text);
  chunks = chunks.filter((c) => {
    const s = String(c || '').trim();
    if (!s || s.length < 6) return false;
    if (/^(?:\d+\s*days?|\d+\s*month[s]?|HMA|VPN|Premium)$/i.test(s)) return false;
    return true;
  });
  const formatted = chunks.map((c) => {
    let t = formatOneAccount(c, hint);
    if (!t) return '';
    // Re-parse labeled result through repair when WhatsApp-shaped
    const labeled = extractLabeled(t);
    const repaired = repairWhatsAppShape(
      { ...labeled, extra: [labeled.extra, String(c)].filter(Boolean).join(' ') },
      hint || description
    );
    // Only accept early repair when WhatsApp shape includes Number (phone)
    if (repaired && repaired.phone && repaired.email && repaired.password) {
      const rebuilt = fieldsToText(repaired);
      if (rebuilt) return rebuilt;
    }
    // Scattered Extra / missing Number — mine phone + emails + mail-pass from full text
    if (!/Number:/i.test(t) && (/Email:/i.test(t) || /Extra:/i.test(t)) && /whatsapp|Format:\s*Number|number\s*[\/|]\s*password/i.test(String(description || ''))) {
      const blob = String(c) + '\n' + String(t);
      const ems = [...blob.matchAll(/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/g)].map((m) => m[0]);
      const uniqE = [];
      const seen = new Set();
      for (const e of ems) {
        const k = e.toLowerCase();
        if (seen.has(k)) continue;
        seen.add(k);
        uniqE.push(e);
      }
      let phone = (blob.match(/\b(1?\d{10})\b/) || [])[1] || (blob.match(/\+(\d{10,15})/) || [])[1];
      let password = (t.match(/Password:\s*(.+)/i) || blob.match(/(\/?Importance[^\s|:]+)/i) || [])[1] || '';
      password = String(password || '').split('\n')[0].trim();
      // email password: short token that is not email/phone/Importance
      const tokens = blob.split(/[\s|:\/]+/).map((x) => x.trim()).filter(Boolean);
      let mailPass = tokens.find((tok) => {
        if (isEmail(tok)) return false;
        if (/importance/i.test(tok)) return false;
        if (/^\d{10,15}$/.test(tok.replace(/\D/g, ''))) return false;
        if (/^(email|password|extra|number|recovery|pass|pwd|username|user)$/i.test(tok)) return false;
        return tok.length >= 6 && tok.length <= 24 && /[a-z]/i.test(tok);
      });
      if (mailPass) mailPass = mailPass.replace(/^\//, '');
      if (phone || uniqE.length) {
        const rebuilt = fieldsToText({
          phone: phone || undefined,
          password: password || undefined,
          email: uniqE[0],
          email_password: mailPass || undefined,
          recovery: uniqE[1] || undefined
        });
        if (rebuilt) return rebuilt;
      }
    }
    return t;
  }).filter(Boolean);
  if (!formatted.length) return text;
  if (formatted.length === 1) return formatted[0];
  return formatted.map((b, i) => String(i + 1) + '. ' + b).join('\n\n');
}

export default formatCredentials;

export function formatMultiLogCredentials(items, formatHint) {
  if (!Array.isArray(items) || items.length === 0) return null;
  const blocks = [];
  for (let i = 0; i < items.length; i++) {
    const item = items[i];
    const raw =
      typeof item === 'string'
        ? item
        : item?.details || item?.credential || item?.login_credentials || '';
    const formatted =
      formatCredentials(String(raw || ''), formatHint) || String(raw || '').trim();
    if (!formatted) continue;
    blocks.push(String(i + 1) + '. ' + formatted);
  }
  return blocks.length ? blocks.join('\n\n') : null;
}

export function joinRawLogDetails(items) {
  if (!Array.isArray(items) || items.length === 0) return null;
  const blocks = [];
  for (let i = 0; i < items.length; i++) {
    const item = items[i];
    const raw =
      typeof item === 'string'
        ? item
        : item?.details || item?.credential || item?.login_credentials || '';
    const s = String(raw || '').trim();
    if (!s) continue;
    blocks.push(String(i + 1) + '. ' + s);
  }
  return blocks.length ? blocks.join('\n\n') : null;
}
