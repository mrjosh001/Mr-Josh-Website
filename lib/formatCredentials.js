...
export function parseFormatHint(description) {
  ...
  if (fmtBlock) {
    ...
    const pairOnly = focus.match(
      /^\s*((?:Number|Phone|Mobile|Username|User|Password|Pass|Email|Uid|Login)(?:\s*[\/|]\s*(?:Number|Phone|Mobile|Username|User|Password|Pass|Email|Uid|Login|2FA|Token))+)/i
    );
...
function extractPhonePasswordLine(text) {
  ...
  const pass = String(m[2] || '').trim();
  if (!pass || isUrl(pass) || isEmail(pass)) return null;
  if (/\b2FA\s*\/\s*Token:/i.test(s) && /\bUsername:/i.test(s)) return null;
  return { phone: m[1].replace(/\s+/g, ' ').trim(), password: pass };
}
...
export function formatOneAccount(raw, formatHint) {
  ...
  if (!text) return '';
  {
    ...
    if (extracted) return fieldsToText({ phone: extracted.phone, password: extracted.password });
