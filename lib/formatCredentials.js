...
function extractPhonePasswordLine(text) {
  ...
  const emailM = rest.match(/([^\s\/|:<>]+@[^\s\/|:<>]+)/);
  if (emailM) {
    const email = emailM[1];
    const before = rest.slice(0, emailM.index).replace(/[\/|:]+$/g, '').trim();
    const after = rest.slice(emailM.index + email.length).replace(/^[\/|:]+/g, '').trim();
    if (before && isEmail(before)) return null;
    return {
      phone,
      password: before || undefined,
      email,
      email_password: after || undefined
    };
  }
...
export function formatCredentials(details, description) {
  ...
  const alreadyLabeled = /^(Email|Username|Password|Number|Phone|Email Password|2FA \/ Token|2FA)\s*:/im.test(text);
  if (!alreadyLabeled) {
    const loose = parseLooseSocialLog(text);
