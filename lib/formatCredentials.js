...
function splitDelimiters(line, expectedCount) {
  ...
  if (t.includes('|')) {
    const segs = t.split('|').map(stripQuotes).filter(Boolean);
    // First segment often email:password — expand so password is its own part
    if (segs.length >= 1) {
      const head = segs[0];
      const em = head.match(/^([^\s:<>]+@[^\s:<>]+)\s*:\s*(.+)$/);
      if (em) {
        return [em[1], em[2], ...segs.slice(1)];
      }
    }
    return segs;
  }
...
/** Fix common inventory shapes: email:pass stuck in Email, Plan= in Password, etc. */
function cleanupFields(f) {
  if (!f || typeof f !== 'object') return f;
  // Email field accidentally holds "user@x.com:password"
  if (f.email && /@/.test(f.email) && String(f.email).includes(':')) {
    const m = String(f.email).match(/^([^\s:<>]+@[^\s:<>]+)\s*:\s*(.+)$/);
    if (m) {
      f.email = m[1];
      const stuck = m[2].trim();
      if (stuck) {
        if (!f.password || /^(Plan|AutoRenew|Expire|DaysLeft|ExpireDate)\b/i.test(String(f.password))) {
          const prev = f.password;
          f.password = stuck.split(/\s*[|]|\s{2,}/)[0].trim() || stuck;
          const more = stuck.includes('|') ? stuck.split('|').slice(1).map((s) => s.trim()).filter(Boolean) : [];
          if (prev && /^(Plan|AutoRenew|Expire|DaysLeft|ExpireDate)\b/i.test(String(prev))) {
            more.unshift(String(prev));
          } else if (prev && prev !== f.password) {
            more.unshift(String(prev));
          }
          if (more.length) f.extra = [more.join(' | '), f.extra].filter(Boolean).join(' | ');
        } else {
          f.extra = [stuck, f.extra].filter(Boolean).join(' | ');
        }
      }
    }
  }
  // Password is metadata (Plan=24Month) not a real password
  if (f.password && /^(Plan|AutoRenew|Expire|DaysLeft|ExpireDate)\b/i.test(String(f.password).trim())) {
    f.extra = [String(f.password).trim(), f.extra].filter(Boolean).join(' | ');
    f.password = '';
  }
  // Split password that still has "realPass | Plan=..."
  if (f.password && String(f.password).includes('|')) {
    const bits = String(f.password).split('|').map((s) => s.trim()).filter(Boolean);
    if (bits.length >= 2) {
      f.password = bits[0];
      f.extra = [bits.slice(1).join(' | '), f.extra].filter(Boolean).join(' | ');
    }
  }
  // Username field holding email
  if (f.username && /@/.test(f.username) && !f.email) {
    f.email = f.username;
    f.username = '';
  }
  return f;
