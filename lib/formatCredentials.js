...
export function parseFormatHint(description) {
  ...
  const slash = text.match(
    /\b((?:USER(?:NAME)?|PASS(?:WORD)?|EMAIL(?:\s*PASS(?:WORD)?)?|2FA|TOKEN|COOKIE|LOGIN|UID)(?:\s*[\/|]\s*(?:USER(?:NAME)?|PASS(?:WORD)?|EMAIL(?:\s*PASS(?:WORD)?)?|2FA|TOKEN|COOKIE|LOGIN|UID))+)/i
  );
...
function splitMultipleAccounts(text) {
  ...
  // Two long digit uids
  const uids = [...t.matchAll(/\b\d{10,}\b/g)];
...
export function formatOneAccount(raw, formatHint) {
  ...
  if (labeled.password || labeled.username || labeled.email || labeled.token) {
    if (labeled.password && /^(https?)$/i.test(labeled.password)) {
      /* keep going */
    } else if (labeled.password || labeled.token || labeled.username || labeled.email) {
  ...
  if (parts && parts.length >= 2) {
    ...
    if (hinted && (hinted.username || hinted.password || hinted.email || hinted.token || hinted.email_password)) {
      // Reclassify leftover so order mistakes get corrected
      const classified = fieldsFromClassified(parts);
      return fieldsToText({
        email: hinted.email || classified.email,
        username: hinted.username || classified.username,
        password: hinted.password || classified.password,
        email_password: hinted.email_password,
        token: hinted.token || classified.token,
        phone: hinted.phone || classified.phone,
        extra: [hinted.extra, classified.extra].filter(Boolean).join(' | ') || classified.extra
      });
    }
    return fieldsToText(fieldsFromClassified(parts));
  }
  ...
  if (lines.length >= 2 && !lines[0].includes(' ')) {
    ...
    return fieldsToText(fieldsFromClassified(lines));
  }
