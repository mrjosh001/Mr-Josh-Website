...
function fieldsToText(f) {
  ...
  if (f.token) {
    ...
    if (cookie) {
      // undo accidental pretty2fa spaces inside cookies
      tok = tok.replace(/\s+/g, '').replace(/c_user=/i, 'c_user=').trim();
      // if spaces destroyed keys, keep original-ish continuous form
      if (!/c_user=/i.test(tok) && /c_user/i.test(String(f.token))) tok = String(f.token).replace(/\s+/g, '');
