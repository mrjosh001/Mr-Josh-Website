...
export function applyApiCors(req, res, { methods = 'GET, POST, OPTIONS' } = {}) {
  ...
  if (allowed.length === 0) {
    ...
    // Explicit wildcard only if you set CORS_ALLOWED_ORIGINS=*
    res.setHeader('Access-Control-Allow-Origin', '*');
  } else if (origin && allowed.includes(origin)) {
    res.setHeader('Access-Control-Allow-Origin', origin);
  } else if (origin && /\.mjhub\.|mjhub\.store$/i.test(origin)) {
    res.setHeader('Access-Control-Allow-Origin', origin);
  }
...
/** Short CDN/browser cache for public-ish GET JSON (catalog). Never use on user money routes. */
export function setCacheHeaders(res, { seconds = 60, swr = 120 } = {}) {
  res.setHeader(
    'Cache-Control',
...
/** 403 if resource.user_id (or similar) is not the JWT user. */
export function assertResourceOwner(jwtUserId, resourceUserId, label = 'resource') {
  if (!jwtUserId || !resourceUserId || String(jwtUserId) !== String(resourceUserId)) {
    return { ok: false, status: 403, message: 'Not allowed to access this ' + label };
  }
  return { ok: true };
}
